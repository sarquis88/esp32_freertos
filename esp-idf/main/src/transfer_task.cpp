...
prvTransferTask( void *pvParameters )
{
    ...
    for( ; ; )
	{
		...
		{
			ESP_LOGI( TRANSFER_LOGGING_TAG, "Connecting..." );
			ESP_ERROR_CHECK( esp_wifi_connect() );
		}

		vTaskDelay( TRANSFER_LONG_DELAY_MS / portTICK_PERIOD_MS);
...
event_handler(void *ctx, system_event_t *event)
{
	...
	{
		ESP_LOGI( TRANSFER_LOGGING_TAG, "IP address: " IPSTR, IP2STR( &event->event_info.got_ip.ip_info.ip ) );
	}
	else if( event->event_id == SYSTEM_EVENT_STA_CONNECTED )
	{
		ESP_LOGI( TRANSFER_LOGGING_TAG, "Connected to %s", (const char *)&event->event_info.connected.ssid );
	}
	else if( event->event_id == SYSTEM_EVENT_STA_DISCONNECTED )
	{
		ESP_LOGI( TRANSFER_LOGGING_TAG, "Disconnected" );
	}
...
wifi_config( string ssid, string passwd )
{
	...
	ESP_LOGI( TRANSFER_LOGGING_TAG, "WiFi configured" );
}
...
socket_config()
{
	...
	ESP_LOGI( TRANSFER_LOGGING_TAG, "Socket configured" );
}
...
send_message( string message )
{
	...
	if( n <= 0 )
		ESP_LOGE( TRANSFER_LOGGING_TAG, "Error sending message to %s", destination_ip );
	else
		ESP_LOGI( TRANSFER_LOGGING_TAG, "Message sended to %s", destination_ip );
}